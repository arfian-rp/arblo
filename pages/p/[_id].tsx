...
export default function PostDetile({ post, isAuth, userToken }: Props) {
  ...
  return (
    <Layout img={post.image!} title={post.title||post.body} description={post.title?`post: ${post.title}\nreply: ${post.replys?.length!}`:`post by ${post.author}\nreply: ${post.replys?.length!}`} isAuth={isAuth} username={userToken?.username}>
